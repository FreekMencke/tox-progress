...
ToxProgressBar.prototype.create = function () {
    ...
    this.element.innerHTML =
        ...
        '   <div class="radial-mask-1" style="transition: transform ' + this.speed + 'ms linear; border-top-left-radius: '+((this.size/2)+2)+'px; border-top-right-radius: '+((this.size/2)+2)+'px; background-color: ' + this.backgroundColor + '; width: ' + (this.size+2) + 'px; height: ' + ((this.size/2)+2) + 'px; top: -' + ((this.size)+2) + 'px;left:-1px;"></div>'  +
        '   <div class="radial-mask-2" style="transition: transform ' + this.speed + 'ms linear; border-top-left-radius: '+((this.size/2)+2)+'px; border-top-right-radius: '+((this.size/2)+2)+'px; background-color: ' + this.backgroundColor + '; width: ' + (this.size+2) + 'px; height: ' + ((this.size/2)+2) + 'px; top: -' + (((this.size)+this.size/2)+4) + 'px;left:-1px;"></div>' +
        ...
        '</div>' +
        (content ? (
        '<div class="' + content.className + '" style="' + (content.dataset.vcenter === 'true'?('display: flex; align-items: center;'):'') + 'top: ' + -this.size + 'px;width:' + this.size + 'px;height:' + this.size + 'px;">' +
...
ToxProgressBar.prototype.animate = function () {
    ...
    if(progress > 50) {
        element.querySelector('.radial-mask-1').style.transition = 'transform ' + (speed/2) + 'ms linear';
        element.querySelector('.radial-mask-2').style.transition = 'transform ' + (speed*((progress-50)/100)) + 'ms linear';
    } else {
        element.querySelector('.radial-mask-1').style.transition = 'transform ' + (speed*(progress/100)) + 'ms linear';
    }
...
var ToxProgress = {
    ...
    init: function () {
        ...
        toxProgressElements.forEach(function (element) {
